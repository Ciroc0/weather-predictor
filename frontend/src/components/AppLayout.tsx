    ...
    <div className="app-shell">
      ...
      <main className="site-main">
        {dashboardQuery.isLoading && !response ? (
          ...
          >
            <Outlet
              context={{ response } satisfies DashboardOutletContext}
            />
          </motion.div>
        ) : (
      ...
      <footer className="site-footer">
        ...
        <span>
          {response?.fetchedAt
            ? `Hentet ${formatDanishDateTime(response.fetchedAt)}`
            : "Afventer data"}{" "}
          · Tidspunkter vises i lokal tid
        </span>

        <span>
          Et projekt af{" "}
          <a href="https://www.mkpdigital.dk/">MKP Digital</a>
        </span>
      </footer>
    </div>
  );
